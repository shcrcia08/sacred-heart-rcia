...
export default function Users() {
  ...
  const handleCycleChange = async (id, newCycleId) => {
    const { error } = await supabase.from('profiles').update({ cycle_id: newCycleId || null }).eq('id', id)
    if (error) setError(error.message)
    else load()
  }
  ...
  return (
    <div>
      ...
      <div className="hint">
        ...
        Each member is permanently tied to whichever cycle was current when they registered — only Admin can change it.
      </div>

      <div className="card">
        {loading ? <p>Loading…</p> : (
          <table>
            <thead><tr><th>Name</th><th>Cycle</th><th>Role</th><th></th></tr></thead>
            <tbody>
              {profiles.map((p) => (
                <tr key={p.id}>
                  <td>{p.full_name}</td>
                  <td>
                    <select value={p.cycle_id ?? ''} onChange={(e) => handleCycleChange(p.id, e.target.value)} style={{ marginBottom: 0 }}>
